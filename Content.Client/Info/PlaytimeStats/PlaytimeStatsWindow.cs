...
public sealed partial class PlaytimeStatsWindow : FancyWindow
{
    ...
    private async void ExportPlaytime()
    {
        ...
        var overallPlaytime = _jobRequirementsManager.FetchOverallPlaytime();
        var rolePlaytimes = _jobRequirementsManager.FetchPlaytimeByRoles().ToDictionary(kv => kv.Key, kv => kv.Value);

        var profile = new PlaytimeInfoProfile(_configurationManager.GetCVar(CVars.BuildForkId), overallPlaytime, rolePlaytimes, _playerManager.LocalUser ?? new NetUserId());
